 ...
 */
  constructor(container, items, groups, options) {
    ...
    this.dom.root.ontouchstart = event => {
      me.emit('touchstart', me.getEventProperties(event))
    };

    this.dom.root.ontouchend = event => {
      me.emit('touchcancel', me.getEventProperties(event))
    };
  ...
  getEventProperties(event) {
    ...
    let clientX, clientY, element, pageX, pageY;

    if (event.touches) {
      if (!event.touches[0]) {
        //console.log ('timeline: touch but no [0]');
        return undefined;
      }
     
      clientX = event.touches[0].clientX ;
      clientY = event.touches[0].clientY;
      element = event.targetTouches[0].target;
      pageX = event.changedTouches[0].pageX;
      pageY = event.changedTouches[0].pageY;
      
      //console.log ('timeline:',clientX, clientY, pageX, pageY);


    } else {
      clientX = event.center ? event.center.x : event.clientX;
      clientY = event.center ? event.center.y : event.clientY;
      element = util.getTarget(event);
